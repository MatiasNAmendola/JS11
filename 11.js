...
(function (compiler) {
	...
	compiler.toJS = function (text) {
		...
		this.prism = text.indexOf("\"use " + "prism\"") > -1 ? true : false

		// Process text
		this.text = text
		this.resetPlaceholderArrays()
		this.strikeImmutableElements()
		this.blockifyIndents()
	...
	compiler.blockifyIndents = function () {
		...
		for (var i = 0; i < lines.length; i++) {
			indent = lines[i].replace(/^(  |\t*).*$/, "$1").replace(/  /, " ").length
			if (indent > previousIndent) {
				for (var j = 0; j < indent - previousIndent; j++) {
					lines[i - 1] += "{"
				}
			} else if (indent < previousIndent) {
				for (j = 0; j < previousIndent - indent; j++) {
					lines[i - 1] += "}"
				}
			}
	...
	compiler.convertMessagingSyntax = function () {
		...
		for (i = 0; i < opens.length; i++) {
			depth = 0
			var colons = new Array,
				simple = true,
				typeCloses = new Array
			for (j = opens[i]; j < this.text.length; j++) {
				// Look for a closer
				if (this.text.charAt(j).match(/[\{\(\[]/) !== null) {
					depth++
				}
				if (this.text.charAt(j).match(/[\}\)\]]/) !== null) {
					depth--
				...
				if (depth === 0) {
					...
					for (k = j; k < this.text.length; k++) {
						if (this.text.charAt(k).match(/[\{\(\[]/) !== null) {
							subdepth++
						}
						if (this.text.charAt(k).match(/[\}\)\]]/) !== null) {
							subdepth--
		...
		for (i = 0; i < closes.length; i++) {
			...
			if (block.simple === true) {
				...
				if (block.type === 2 && !block.hasHandler) {
					block.type = 3
				}

				var parts, more, target, constructor, argString, args = new Array,
					properties, method
		...
		function blockType(block, startIndex, endIndex) {
			if (arguments.length === 1 && typeof arguments[0] === "object" && arguments[0]._ === null) {
				endIndex = "endIndex" in arguments[0] ? arguments[0].endIndex : 0;
				startIndex = "startIndex" in arguments[0] ? arguments[0].startIndex : 0;
				block = "block" in arguments[0] ? arguments[0].block : ""
			} else {
...
if (typeof window === "undefined" && typeof global === "undefined") {
	...
	this.onmessage = function (event) {
			if (arguments.length === 1 && typeof arguments[0] === "object" && arguments[0]._ === null) {
				event = "event" in arguments[0] ? arguments[0].event : {
					data: ""
				}
			} else {
